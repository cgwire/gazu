...
class ProjectTestCase(unittest.TestCase):
    ...
    def test_all_open_projects(self):
        with requests_mock.mock() as mock:
            mock.get(
                pipeline.client.get_full_url('data/user/projects/open'),
                text='[{"name": "Big Buck Bunny", "id": "project_1"}]'
            )
            projects = pipeline.user.all_open_projects()
    ...
    def test_asset_types_for_project(self):
        with requests_mock.mock() as mock:
            ...
            asset_types = pipeline.user.all_asset_types_for_project(project)
    ...
    def test_asset_for_asset_type_and_project(self):
        with requests_mock.mock() as mock:
            ...
            assets = pipeline.user.all_assets_for_asset_type_and_project(
                project,
                asset_type
            )
            asset = assets[0]
    ...
    def test_tasks_for_asset(self):
        with requests_mock.mock() as mock:
            ...
            asset = {"id": "asset-1"}
            tasks = pipeline.user.all_tasks_for_asset(asset)
            task = tasks[0]
    ...
    def test_tasks_for_shot(self):
        with requests_mock.mock() as mock:
            ...
            shot = {"id": "shot-1"}
            tasks = pipeline.user.all_tasks_for_shot(shot)
            task = tasks[0]
            self.assertEquals(task["name"], "main")
    ...
    def test_task_types_for_asset(self):
        with requests_mock.mock() as mock:
            ...
            asset = {"id": "asset-1"}
            task_types = pipeline.user.all_task_types_for_asset(asset)
    ...
    def test_task_types_for_shot(self):
        with requests_mock.mock() as mock:
            ...
            shot = {"id": "shot-1"}
            tasks = pipeline.user.all_task_types_for_shot(shot)
            task = tasks[0]
    ...
    def test_sequences_for_project(self):
        with requests_mock.mock() as mock:
            ...
            project = {"id": "project-1"}
            sequences = pipeline.user.all_sequences_for_project(project)
    ...
    def test_shot_for_sequences(self):
        with requests_mock.mock() as mock:
            ...
            shots = pipeline.user.all_shots_for_sequence(sequence)
            shot = shots[0]
