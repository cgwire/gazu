...
class TaskTestCase(unittest.TestCase):
    ...
    def test_all_playlists_for_project(self):
        with requests_mock.mock() as mock:
            mock.get(
                gazu.client.get_full_url("data/projects/%s/playlists" % fakeid("project-1")),
                text=json.dumps(
                    [
                        {"id": fakeid('playlist-1'), "name": "playlist-1"},
                        {"id": fakeid('playlist-2'), "name": "playlist-2"},
    ...
    def test_all_playlists_for_episode(self):
        with requests_mock.mock() as mock:
            mock.get(
                gazu.client.get_full_url("data/projects/%s/episodes/%s/playlists" % (fakeid("project-1"), fakeid("episode-1"))),
                text=json.dumps(
                    [
                        {"id": fakeid('playlist-1'), "name": "playlist-1"},
                        {"id": fakeid('playlist-2'), "name": "playlist-2"},
    ...
    def test_get_playlist_by_name(self):
        with requests_mock.mock() as mock:
            mock.get(
                gazu.client.get_full_url("data/playlists?project_id=%s&name=playlist-1" % fakeid("project-1")),
                text=json.dumps([
                    {"id": fakeid("playlist-1"),
                    "name": "playlist-1",}
                ])
            )
            self.assertEqual(gazu.playlist.get_playlist_by_name(fakeid("project-1"), "playlist-1")['id'], fakeid('playlist-1'))
